...
import { asignarRecetaACalendario } from "@/features/dashboard/calendario/service/calendarioService";
...
const mockAsignar = asignarRecetaACalendario as jest.Mock;
...
describe("useAsignarReceta", () => {
    ...
    describe("carga inicial", () => {
        it("hace fetch inicial con tab plataforma y página 1", async () => {
            const { result } = renderHook(() => useAsignarReceta("2026-01-01", 1, jest.fn()));
            await waitFor(() => {
    ...
    describe("paginación acumulativa", () => {
        it("acumula recetas al cargar más cuando hay más páginas", async () => {
            ...
            mockObtenerRecetas.mockResolvedValue({ recipeDto: [receta2], totalPages: 2, totalRecords: 20 });
    ...
    describe("handleAsignar", () => {
        it("no hace nada si no hay receta seleccionada", async () => {
            const onAsignado = jest.fn();
            const { result } = renderHook(() => useAsignarReceta("2026-01-01", 1, onAsignado));
            await waitFor(() => expect(result.current.recetas).toEqual([receta1]));

            await act(async () => { await result.current.handleAsignar(); });
        ...
        it("asigna la receta, notifica éxito y llama a onAsignado", async () => {
            const onAsignado = jest.fn();
            const { result } = renderHook(() => useAsignarReceta("2026-01-01", 1, onAsignado));
            await waitFor(() => expect(result.current.recetas).toEqual([receta1]));
            ...
            expect(mockAsignar).toHaveBeenCalledWith({ fecha: "2026-01-01", tipo_comida_id: 1, receta_id: 2 });
            expect(mockMostrarNotificacion).toHaveBeenCalledWith('Receta "Ensalada" asignada correctamente.', "success");
            expect(onAsignado).toHaveBeenCalled();
        });

        it("notifica error y no llama a onAsignado si falla la asignación", async () => {
            mockAsignar.mockRejectedValue(new Error("No se pudo asignar"));
            const onAsignado = jest.fn();
            const { result } = renderHook(() => useAsignarReceta("2026-01-01", 1, onAsignado));
            await waitFor(() => expect(result.current.recetas).toEqual([receta1]));
            ...
            expect(mockMostrarNotificacion).toHaveBeenCalledWith("Error al asignar la receta.", "error");
            expect(onAsignado).not.toHaveBeenCalled();
        });
    });
